...
DD-FR-001で上記semantic lifecycleは決定した。DD-FR-002のnumeric layout、checksum profile、exact create/open/share/access flags、eligible filesystem、MachineActorRecord serialization、worker one-shot negative oracleはPhase 1A blockerではないが、Phase 2A code/file作成前にfreezeする。Phase 2A実機/fault evidenceが不成立なcellはPhase 1B No-Goである。
