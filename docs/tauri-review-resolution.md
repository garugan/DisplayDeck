...
- **residual risk**: CANDIDATE-04のexact bytes/hash/indexとindependent static reviewはCLEANだが、human freeze approval、Reviewer/Approver、immutable approval referenceは未記録である。D07は`DIRECTORY_ANCHOR_UNPROVEN / NO_GO_RECORDED`、D08はactive/sleep-resume合計10件のboot tuple/query内metricsが整合したがcross-sleep tick/UTC advance、restart sample、tolerance evidence、formal bundleはpending、G1Aはtemplate/validatorのみでformal result evidence pendingである。Windows filesystem/storage/filter/power、DACL、old binary behaviorはP2A/8 evidenceでNo-Go判定する。
