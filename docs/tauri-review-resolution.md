...
- **residual risk**: CANDIDATE-04のexact bytes/hash/indexとindependent static reviewはCLEANだが、human freeze approval、Reviewer/Approver、immutable approval referenceは未記録である。D07は`DIRECTORY_ANCHOR_UNPROVEN / NO_GO_RECORDED`、D08は最初のWindows read-only captureが外部validatorを通過したが複数sample/tolerance evidenceとformal bundleはpending、G1Aはtemplate/validatorのみでformal result evidence pendingである。Windows filesystem/storage/filter/power、DACL、old binary behaviorはP2A/8 evidenceでNo-Go判定する。
