...
- **residual risk**: CANDIDATE-04のexact bytes/hash/indexとindependent static reviewはCLEANだが、human freeze approval、Reviewer/Approver、immutable approval referenceは未記録である。D07は`DIRECTORY_ANCHOR_UNPROVEN / NO_GO_RECORDED`、D08は同一active session 5件のboot tuple/metricsが整合したがsleep/restart sample、tolerance evidence、formal bundleはpending、G1Aはtemplate/validatorのみでformal result evidence pendingである。Windows filesystem/storage/filter/power、DACL、old binary behaviorはP2A/8 evidenceでNo-Go判定する。
