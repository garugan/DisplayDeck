...
- **residual risk**: CANDIDATE-04のexact bytes/hash/indexとindependent static reviewはCLEANだが、human freeze approval、Reviewer/Approver、immutable approval referenceは未記録である。D07は`DIRECTORY_ANCHOR_UNPROVEN / NO_GO_RECORDED`、D08はactive/sleep-resume/restart合計15件、cross-sleep tick/UTC advance、sleepでBootTime不変、full restartでBootTime/BootId変化とtick reset、restart内boot tuple 5/5一致まで観測した。hibernate evidence、tolerance evidence、formal bundleはpendingである。G1Aはtemplate/validatorのみでformal result evidence pendingである。Windows filesystem/storage/filter/power、DACL、old binary behaviorはP2A/8 evidenceでNo-Go判定する。
