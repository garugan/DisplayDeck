...
- D08 cross-check candidateのsampling orderは`t0=GetTickCount64 -> u0=precise UTC FILETIME -> WMI read/strict parse -> u1=precise UTC FILETIME -> t1=GetTickCount64`とし、`predictedBoot0=u0-(t0*10000)`、`predictedBoot1=u1-(t1*10000)`をchecked arithmeticで計算する。
- 2026-08-24の文書候補`DD-FR-002-D08-TOLERANCE-CANDIDATE-01`は、`maxBootIdentitySampleSpanMs=250`、`maxBootUtcDelta100ns=500000`（50 ms）、clock-jump rule=`REJECT_NON_MONOTONIC_OR_PREDICTED_BOOT_DELTA_EXCEEDS_LIMIT`をrecommended candidateとする。根拠となるWindows 10 `10.0.19045`のread-only観測は、active / sleep-resume / full restart / hibernate前後の25件で最大sample span=`110 ms`、最大`abs(predictedBoot1-predictedBoot0)`=`13.977 ms`、same-boot cross-transitionの最大tick/UTC advance差=`5.588 ms`だった。250 msは観測最大sample spanの約2.27倍、50 msは観測最大predicted-boot差の約3.58倍である。
- CANDIDATE-01のlocal capture ruleは、checked arithmeticで`t1 >= t0`、`u1 > u0`、`t1-t0 <= 250`、`abs(predictedBoot1-predictedBoot0) <= 500000`を全て満たす場合だけcross-checkをcandidate-passとする。逆行、overflow/underflow、上限超過、WMI strict-parse不一致、Version/Build不一致、provider errorは`BOOT_IDENTITY_UNPROVEN`としてsame-boot authorityを発行せず、wall clockでdeadlineを延長または再構成しない。rebootでBootIdが変化しtick resetした観測はnew-boot boundaryであり、このsame-boot ruleのpassへ変換しない。
- CANDIDATE-01は一台のWindows 10実機から得たlab candidateで、runtime templateの3 fieldは引き続き`UNSET`、capture resultは`ACCEPTANCE_NOT_AUTHORIZED`とし、WMI timeout/retry/provider-error policyも`UNSET/EVIDENCE_PENDING`のままにする。高速スタートアップ有効cell、Windows 11、対象CPU architecture、負荷/security-product差、WMI timeout/retry/provider-error、clock-change境界、threshold直下/一致/直上のevidence、Reviewer/Approver/immutable approval referenceが揃うまで`FROZEN`またはproduction toleranceへ昇格しない。
...
| `DD-FR-002-D08` | boot identity preimage / cross-check | stable WMI boot UTC + parsed version/buildだけをdomain-separated hashへ入れ、tick/UTC sampleは別acceptance evidenceとして実機でtoleranceをfreeze | sample-dependent値をhashするとactor間で再現不能。CANDIDATE-01は一台のWindows 10実機だけに基づくため、追加cell evidenceなしに固定しない | `POLICY_APPROVED / SPEC_CANDIDATE / READ_ONLY_AUTHORIZED / ACTIVE_SLEEP_RESTART_HIBERNATE_BATCHES_25_OF_25_IDENTITY_METRICS_CONSISTENT / CROSS_SLEEP_HIBERNATE_TICK_UTC_ADVANCE_CONSISTENT / RESTART_BOOT_BOUNDARY_CONFIRMED / HIBERNATE_SAME_BOOT_OBSERVED / FAST_STARTUP_DISABLED_OBSERVED / FAST_STARTUP_ENABLED_CELL_NOT_TESTED / TOLERANCE_CANDIDATE_01_DRAFTED / TOLERANCE_EVIDENCE_PENDING / HUMAN_FREEZE_APPROVAL_PENDING` |
