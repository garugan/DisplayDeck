...
2026-08-24の文書候補`DD-FR-002-D08-TOLERANCE-CANDIDATE-01`は、`maxBootIdentitySampleSpanMs=250`、`maxBootUtcDelta100ns=500000`（50 ms）、clock-jump rule=`REJECT_NON_MONOTONIC_OR_PREDICTED_BOOT_DELTA_EXCEEDS_LIMIT`です。local captureでtick/UTCが逆行せず、sample spanが250 ms以下、2つのpredicted bootの差が50 ms以下の場合だけcandidate-passとし、それ以外は`BOOT_IDENTITY_UNPROVEN`として扱います。これは一台のWindows 10実機から作ったlab candidateで、JSON templateは`UNSET`、実機captureは`ACCEPTANCE_NOT_AUTHORIZED`のままです。追加cell evidenceと人間のfreeze承認が揃うまでproduction値ではありません。
...
次は現在のPowerShell sessionで`$activeBatch = $batch`を実行してから、operatorが手動でWindowsをsleep/resumeします。helper自身はpower transitionを起動しません。resume後は次の5回batchを実行します。

```powershell
$activeBatch = $batch

# この行の後、operatorが手動でsleepし、復帰後に以下を続ける。
$label = "sleep-resume"
$batch = Join-Path $env:TEMP ("displaydeck-d08-{0}-{1}" -f $label, (Get-Date -Format "yyyyMMdd-HHmmss"))
New-Item -ItemType Directory -Path $batch -ErrorAction Stop | Out-Null
1..5 | ForEach-Object {
